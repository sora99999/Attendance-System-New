...
async function loadData() {
    const result = await apiGet('get_data');
    if (result.success) {
        ...
        alert('Failed to load data from database. Please check your database connection.');
    }
}
...
window.setStatus = async (studentId, status) => {
    ...
    if (appData.viewingDate) {
        ...
        if (student) {
            ...
            // Update in database
            await apiCall('update_student_status', {
                id: studentId,
                status: newStatus
            });
...
window.removeStudent = async (studentId) => {
    ...
    const section = getCurrentSection();
    const result = await apiCall('remove_student', { id: studentId });
...
window.deleteSection = async (sectionId) => {
    if (confirm("Delete this section permanently?")) {
        const result = await apiCall('delete_section', { id: sectionId });
...
els.lockBtn.addEventListener('click', async () => {
    ...
    if (section.isLocked) {
        ...
        const result = await apiCall('save_session', {
...
window.saveHistoryChanges = async () => {
    ...
    if (node) {
        const newModCount = (node.modCount || 0) + 1;

        const result = await apiCall('update_history', {
            sectionId: section.id,
            sessionName: appData.viewingDate,
            records: node.data,
            modCount: newModCount
        });

        if (result.success) {
            node.modCount = newModCount;

            if (newModCount >= 2) {
                alert("Changes saved. \n\nYou have reached the limit of 2 edits for this session. It is now permanently locked.");
            } else {
                alert(`Changes saved. \n\nYou have ${2 - newModCount} edits remaining.`);
            }
        } else {
...
els.deleteSectionFullBtn.addEventListener('click', async () => {
    const section = getCurrentSection();
    if (confirm(`DELETE section "${section.name}" forever?`)) {
        const result = await apiCall('delete_section', { id: section.id });
...
els.confirmAddSection.addEventListener('click', async () => {
    ...
    const result = await apiCall('create_section', {
        id: newSec.id,
        name: newSec.name,
        subject: newSec.subject,
        createdAt: newSec.createdAt
    });
    ...
    if (result.success) {
        ...
        alert('Failed to create section: ' + result.error);
    }
});

els.confirmAddStudent.addEventListener('click', async () => {
    ...
    const result = await apiCall('add_student', {
        id: studentId,
        sectionId: section.id,
        name: name
    });
